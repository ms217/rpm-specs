...
Source0: getmail6-%{version}.tar.gz
BuildArch: noarch
BuildRequires: python3-devel
Obsoletes: getmail < %{version}-%{release}
...
%prep
%setup -q
sed -i 's/#!\/usr\/bin\/env.*$/#!\/usr\/bin\/python3/' getmail getmailcore/*.py

%build

%install
python3 setup.py install --root="%{buildroot}"
chmod a+x %{buildroot}/%{python3_sitelib}/getmailcore/*.py
chmod a-x %{buildroot}/%{python3_sitelib}/getmailcore/imap_utf7.py
rm docs/*.1
rm -rf %{buildroot}/%{_docdir}/%{name}-%{version}

%files
%{_docdir}/%{_doc_name}-%{version}/*
%{_mandir}/man1/getmails.1*
...
%{python3_sitelib}/getmail*.egg-info
%{python3_sitelib}/getmailcore/

%changelog
* Thu Apr 04 2024 Michael Seevogel <michael@michaelseevogel.de> - 6.18.14-1
- Update to Getmail v6

* Sun Nov 14 2021 Michael Seevogel <michael@michaelseevogel.de> - 5.16-1
- upstream 5.16 release
- No code changes, just documentation-only changes.

* Mon Apr 05 2021 Michael Seevogel <michael@michaelseevogel.de> - 5.15-1
- upstream 5.15 release
- No code changes, just documentation-only changes.

* Mon May 11 2020 Michael Seevogel <michael@michaelseevogel.de> - 5.14-1
- upstream 5.14 release
