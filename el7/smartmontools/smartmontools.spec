...
autoreconf -i
%if %{with_systemd}
%configure --with-selinux --without-nvme-devicescan --with-initscriptdir=no --with-libcap-ng=yes --with-systemdsystemunitdir=%{_unitdir} --sysconfdir=%{_sysconfdir}/%name/
%else
%configure --with-selinux --without-nvme-devicescan --with-initscriptdir=no --with-libcap-ng=yes --with-systemdsystemunitdir=no --sysconfdir=%{_sysconfdir}/%name/
%endif
%ifarch sparc64
...
%if !%{with_systemd}
install -D -m 755 %{SOURCE5} $RPM_BUILD_ROOT/%{_initrddir}/smartd
%endif
...
%changelog
* Wed Jul 20 2016 Michael Seevogel <ms@ddnetservice.net> - 1:6.5-1
- Updated smartmontools to 6.5
- disabled NVME feature via --without-nvme-devicescan because it is still experimental
- updated spec file so that it's possible to build this package for init.d and systemd systems
